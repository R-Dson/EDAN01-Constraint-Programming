...
public class SplitSearchTask2 {
    ...
    public SplitSearchTask2(Store s) {
        store = s;
    }
    ...
    public boolean label(IntVar[] vars) {
        ...
        if (!consistent) {
            ...
            //store.impose(choice.halfDomain());

            // choice point imposed.
            // choice.var.setDomain(1, 2);
            ...
            if (consistent) {
                ...
                //store.impose(new Not(choice.halfDomain()));
    ...
    public class ChoicePoint {
        ...
        IntVar selectVariable(IntVar[] v) {
            if (v.length != 0) {
                // Get the domain min and max values
                IntDomain totalDomain = v[v.length-1].dom();

                // If the domain is reduced to a single value
                // The reason is that we keep moving the 'value' variable to the middle of a new half domain
                if (totalDomain.singleton()){
        ...
        int selectValue(IntVar v) {
            IntDomain totalDomain = var.domain;
            int mid = (int)Math.ceil((float)(totalDomain.max() + totalDomain.min()) / 2);
            return mid;
            //return v.min();
        }
        ...
        public PrimitiveConstraint halfDomain(){
            //IntDomain totalDomain = var.domain;
            //int mid = (totalDomain.max() + totalDomain.min()) / 2;
